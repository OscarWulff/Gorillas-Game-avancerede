...
public class City {
        ...
        public City(int height, int width) { //In this method I initialize the City map with the buildings
            ...
            buildings.add(new Building(9, 230, maxHeight - 310 + 25, maxHeight));
            buildings.add(new Building(230, 388, maxHeight - 468 + 25, maxHeight));
            buildings.add(new Building(388, 609, maxHeight - 340 + 25, maxHeight));
            buildings.add(new Building(604, 833, maxHeight - 690 + 25, maxHeight));
            buildings.add(new Building(833, 1002, maxHeight - 431 + 25, maxHeight));
            buildings.add(new Building(1000, 1248, maxHeight - 569 + 25, maxHeight));
            buildings.add(new Building(1243, 1505, maxHeight - 340 + 25, maxHeight));
            buildings.add(new Building(1505, 1700, maxHeight - 501 + 25, maxHeight));
        ...
        public void hitBoxbuildings() { //This method sets the grid to true for every building in the city map.
            for (int i = buildings.get(0).getStart_y(); i < buildings.get(0).getEnd_y(); i++) { //First building start y and end y coordinates.
                for (int k = buildings.get(0).getStart_x(); k < buildings.get(0).getEnd_x(); k++) { //First buildings start x and end x coordinates.
                    if (i >= 0 && k >= 0 && i < maxHeight && k < maxWidth) { //An if-statement that narrow it down to only building 1's pixels.
